...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed partial class BoundFixedSizeBufferElementAccess : BoundExpression
    {
        public BoundFixedSizeBufferElementAccess(SyntaxNode syntax, BoundExpression expression, BoundExpression argument, bool isValue, MethodSymbol? getSpanOrReadOnlySpanHelper, WellKnownMember getItemOrSliceHelper, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.FixedSizeBufferElementAccess, syntax, type, hasErrors || expression.HasErrors() || argument.HasErrors())
        {
            ...
            this.GetSpanOrReadOnlySpanHelper = getSpanOrReadOnlySpanHelper;
        ...
        public MethodSymbol? GetSpanOrReadOnlySpanHelper { get; }
        public WellKnownMember GetItemOrSliceHelper { get; }

        [DebuggerStepThrough]
        public override BoundNode? Accept(BoundTreeVisitor visitor) => visitor.VisitFixedSizeBufferElementAccess(this);

        public BoundFixedSizeBufferElementAccess Update(BoundExpression expression, BoundExpression argument, bool isValue, MethodSymbol? getSpanOrReadOnlySpanHelper, WellKnownMember getItemOrSliceHelper, TypeSymbol type)
        {
            if (expression != this.Expression || argument != this.Argument || isValue != this.IsValue || !Symbols.SymbolEqualityComparer.ConsiderEverything.Equals(getSpanOrReadOnlySpanHelper, this.GetSpanOrReadOnlySpanHelper) || getItemOrSliceHelper != this.GetItemOrSliceHelper || !TypeSymbol.Equals(type, this.Type, TypeCompareKind.ConsiderEverything))
            {
                var result = new BoundFixedSizeBufferElementAccess(this.Syntax, expression, argument, isValue, getSpanOrReadOnlySpanHelper, getItemOrSliceHelper, type, this.HasErrors);
                result.CopyAttributes(this);
                return result;
            }
            return this;
        ...
        internal R VisitInternal(BoundNode node, A arg)
        {
            switch (node.Kind)
            {
                ...
                case BoundKind.FixedSizeBufferElementAccess:
                    return VisitFixedSizeBufferElementAccess((BoundFixedSizeBufferElementAccess)node, arg);
    ...
    internal abstract partial class BoundTreeVisitor<A, R>
    {
        ...
        public virtual R VisitFixedSizeBufferElementAccess(BoundFixedSizeBufferElementAccess node, A arg) => this.DefaultVisit(node, arg);
    ...
    internal abstract partial class BoundTreeVisitor
    {
        ...
        public virtual BoundNode? VisitFixedSizeBufferElementAccess(BoundFixedSizeBufferElementAccess node) => this.DefaultVisit(node);
    ...
    internal abstract partial class BoundTreeRewriter : BoundTreeVisitor
    {
        ...
        public override BoundNode? VisitFixedSizeBufferElementAccess(BoundFixedSizeBufferElementAccess node)
        {
            ...
            BoundExpression argument = (BoundExpression)this.Visit(node.Argument);
            TypeSymbol? type = this.VisitType(node.Type);
            return node.Update(expression, argument, node.IsValue, node.GetSpanOrReadOnlySpanHelper, node.GetItemOrSliceHelper, type);
        }
        public override BoundNode? VisitDynamicIndexerAccess(BoundDynamicIndexerAccess node)
        {
            BoundExpression receiver = (BoundExpression)this.Visit(node.Receiver);
    ...
    internal sealed partial class NullabilityRewriter : BoundTreeRewriter
    {
        ...
        public override BoundNode? VisitFixedSizeBufferElementAccess(BoundFixedSizeBufferElementAccess node)
        {
            MethodSymbol? getSpanOrReadOnlySpanHelper = GetUpdatedSymbol(node, node.GetSpanOrReadOnlySpanHelper);
            BoundExpression expression = (BoundExpression)this.Visit(node.Expression);
            BoundExpression argument = (BoundExpression)this.Visit(node.Argument);
            BoundFixedSizeBufferElementAccess updatedNode;

            if (_updatedNullabilities.TryGetValue(node, out (NullabilityInfo Info, TypeSymbol? Type) infoAndType))
            {
                updatedNode = node.Update(expression, argument, node.IsValue, getSpanOrReadOnlySpanHelper, node.GetItemOrSliceHelper, infoAndType.Type!);
                updatedNode.TopLevelNullability = infoAndType.Info;
            }
            else
            {
                updatedNode = node.Update(expression, argument, node.IsValue, getSpanOrReadOnlySpanHelper, node.GetItemOrSliceHelper, node.Type);
            }
            return updatedNode;
        }
    ...
    internal sealed class BoundTreeDumperNodeProducer : BoundTreeVisitor<object?, TreeDumperNode>
    {
        ...
        public override TreeDumperNode VisitFixedSizeBufferElementAccess(BoundFixedSizeBufferElementAccess node, object? arg) => new TreeDumperNode("fixedSizeBufferElementAccess", null, new TreeDumperNode[]
        {
            new TreeDumperNode("expression", null, new TreeDumperNode[] { Visit(node.Expression, null) }),
            new TreeDumperNode("argument", null, new TreeDumperNode[] { Visit(node.Argument, null) }),
            new TreeDumperNode("isValue", node.IsValue, null),
            new TreeDumperNode("getSpanOrReadOnlySpanHelper", node.GetSpanOrReadOnlySpanHelper, null),
