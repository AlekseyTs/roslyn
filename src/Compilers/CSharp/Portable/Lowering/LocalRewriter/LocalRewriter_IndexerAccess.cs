...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class LocalRewriter
    {
        ...
        {
            Debug.Assert(node.GetSpanOrReadOnlySpanHelper is object);
            ...
            getItemOrSliceHelper = getItemOrSliceHelper.AsMember((NamedTypeSymbol)node.GetSpanOrReadOnlySpanHelper.ReturnType);

            BoundExpression result;

            if (node.Argument.Type.SpecialType == SpecialType.System_Int32)
            {
                result = _factory.Call(_factory.Call(rewrittenReceiver, node.GetSpanOrReadOnlySpanHelper), getItemOrSliceHelper, VisitExpression(node.Argument));
            }
            else
            {
                _ = node.Expression.Type.IsSafeFixedSizeBuffer(out int length);
                ...
                if (TypeSymbol.Equals(node.Argument.Type, _compilation.GetWellKnownType(WellKnownType.System_Index), TypeCompareKind.AllIgnoreOptions))
                {
                    ...
                    result = _factory.Call(_factory.Call(rewrittenReceiver, node.GetSpanOrReadOnlySpanHelper), getItemOrSliceHelper, integerArgument);
                }
                else
                {
                    ...
                    if (rangeExpr is not null)
                    {
                        ...
                        // PROTOTYPE(SafeFixedSizeBuffers): Omit Slice call when arguments are 0 and length.
                    }
                    else
                    {
                    ...
                    result = _factory.Sequence(localsBuilder.ToImmutableAndFree(), sideEffectsBuilder.ToImmutableAndFree(),
                                               _factory.Call(_factory.Call(possiblyRefCapturedReceiver, node.GetSpanOrReadOnlySpanHelper),
        ...
        private BoundExpression MakeRangeSize(ref BoundExpression startExpr, BoundExpression endExpr, ArrayBuilder<LocalSymbol> localsBuilder, ArrayBuilder<BoundExpression> sideEffectsBuilder)
        {
            ...
            else
            {
                ...
                {
                    Debug.Assert(startExpr is not BoundLocal { LocalSymbol.SynthesizedKind: not SynthesizedLocalKind.UserDefined }); // Might want to avoid a copy otherwise
