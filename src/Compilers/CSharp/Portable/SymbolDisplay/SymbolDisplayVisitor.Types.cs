...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal partial class SymbolDisplayVisitor
    {
        ...
        private void AddNameAndTypeArgumentsOrParameters(INamedTypeSymbol symbol)
        {
            ...
            TypeSymbol underlyingTypeSymbol = (symbol as Symbols.PublicModel.NamedTypeSymbol)?.UnderlyingTypeSymbol;
            ...
            {
                symbol = illegalGenericInstantiationSymbol.UnderlyingSymbol.GetPublicSymbol<INamedTypeSymbol>();
            }
            else
            {
                ...
                {
                    symbol = ambiguousCanonicalTypeSymbol.FirstCandidate.GetPublicSymbol<INamedTypeSymbol>();
                }
                else
                {
            ...
            if (format.CompilerInternalOptions.IncludesOption(SymbolDisplayCompilerInternalOptions.UseArityForGenericTypes))
            {
                ...
                if ((((ISymbolInternalSource)symbol).GetISymbolInternal() as NamedTypeSymbol)?.MangleName == true)
                {
            ...
            else if (symbol.Arity > 0 && format.GenericsOptions.IncludesOption(SymbolDisplayGenericsOptions.IncludeTypeParameters))
            {
                ...
                if (underlyingTypeSymbol is UnsupportedMetadataTypeSymbol ||
                ...
                else
                {
                    ...
                    {
                        var namedType = underlyingTypeSymbol as NamedTypeSymbol;
