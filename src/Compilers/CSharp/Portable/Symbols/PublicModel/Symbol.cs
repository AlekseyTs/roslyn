...
{
    internal abstract class Symbol : ISymbol, ISymbolInternalSource
    {
        ...
        protected static ImmutableArray<TypeWithAnnotations> ConstructTypeArguments(ITypeSymbol[] typeArguments)
        {
            ...
            {
                var type = typeArg.EnsureCSharpSymbolOrNull<ITypeSymbol, Symbols.PublicModel.TypeSymbol>(nameof(typeArguments))?.UnderlyingTypeSymbol;
        ...
        protected static ImmutableArray<TypeWithAnnotations> ConstructTypeArguments(ImmutableArray<ITypeSymbol> typeArguments, ImmutableArray<CodeAnalysis.NullableAnnotation> typeArgumentNullableAnnotations)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                var type = typeArguments[i].EnsureCSharpSymbolOrNull<ITypeSymbol, Symbols.PublicModel.TypeSymbol>(nameof(typeArguments))?.UnderlyingTypeSymbol;
        ...
        ISymbol ISymbol.OriginalDefinition
        {
            get
            {
                return UnderlyingSymbol.OriginalDefinition.GetPublicSymbol<ISymbol>();
            }
        }
        ...
        ISymbol ISymbol.ContainingSymbol
        {
            get
            {
                return UnderlyingSymbol.ContainingSymbol.GetPublicSymbol<ISymbol>();
            }
        }
        ...
        INamedTypeSymbol ISymbol.ContainingType
        {
            get
            {
                return UnderlyingSymbol.ContainingType.GetPublicSymbol<INamedTypeSymbol>();
            }
        }
        ...
        IAssemblySymbol ISymbol.ContainingAssembly => UnderlyingSymbol.ContainingAssembly.GetPublicSymbol<IAssemblySymbol>();

        IModuleSymbol ISymbol.ContainingModule => UnderlyingSymbol.ContainingModule.GetPublicSymbol<IModuleSymbol>();

        INamespaceSymbol ISymbol.ContainingNamespace => UnderlyingSymbol.ContainingNamespace.GetPublicSymbol<INamespaceSymbol>();
        ...
        ISymbolInternal ISymbolInternalSource.GetISymbolInternal()
        {
