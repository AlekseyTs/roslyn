...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        private WeakReference<ISymbol> _lazyAndWeakISymbol;
        ...
        internal ISymbol ISymbol
        {
            get
            {
                ...
                while (true)
                {
                    WeakReference<ISymbol> lazyAndWeakISymbol = _lazyAndWeakISymbol;

                    if (lazyAndWeakISymbol is null)
                    {
                        result = CreateISymbol();
                        lazyAndWeakISymbol = Interlocked.CompareExchange(ref _lazyAndWeakISymbol, new WeakReference<ISymbol>(result), null);
                        if (lazyAndWeakISymbol is null)
                        {
                            return result;
                        }
                    ...
                    if (lazyAndWeakISymbol.TryGetTarget(out result))
                    {
                        return result;
                    }

                    Interlocked.CompareExchange(ref _lazyAndWeakISymbol, null, lazyAndWeakISymbol);
                }
            }
        }
