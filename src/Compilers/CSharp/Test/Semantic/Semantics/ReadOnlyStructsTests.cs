...
";
            ...
            void testStatement(StatementSyntax statementSyntax, bool isEffectivelyReadOnly)
            {
                ...
                Assert.Equal(isEffectivelyReadOnly, symbol.GetSymbol<MethodSymbol>().IsEffectivelyReadOnly);
                Assert.Equal(isEffectivelyReadOnly, symbol.IsReadOnly);

                Assert.False(symbol.GetSymbol<MethodSymbol>().IsDeclaredReadOnly);
                Assert.False(reducedFrom.GetSymbol<MethodSymbol>().IsDeclaredReadOnly);
                Assert.False(reducedFrom.GetSymbol<MethodSymbol>().IsEffectivelyReadOnly);
...
";
            ...
            static void check(PropertySymbol property, bool returnsByRef, bool returnsByRefReadonly, bool isReadOnly)
            {
                ...
                Assert.Equal(isReadOnly, property.GetMethod.GetPublicSymbol<IMethodSymbol>().IsReadOnly);
            }
        }
