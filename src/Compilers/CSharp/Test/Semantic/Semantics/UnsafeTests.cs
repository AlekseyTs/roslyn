...
";
            ...
            Assert.True(tree.GetCompilationUnitRoot().DescendantNodes().OfType<AnonymousObjectCreationExpressionSyntax>().
                Select(syntax => model.GetTypeInfo(syntax).Type).All(type => type.GetSymbol<TypeSymbol>().IsManagedType));
        }
...
";
            ...
            Assert.True(methodGroupSummary.MethodGroup.SetEquals(ImmutableArray.Create<IMethodSymbol>(structMethod1.GetPublicSymbol<IMethodSymbol>(), structMethod2.GetPublicSymbol<IMethodSymbol>()), EqualityComparer<IMethodSymbol>.Default));
...
";
            ...
            Assert.Null(methodGroupSummary.ConvertedType);
            Assert.Equal(ConversionKind.Identity, methodGroupSummary.ImplicitConversion.Kind);
            Assert.True(methodGroupSummary.MethodGroup.SetEquals(structMethods.SelectAsArray(m => m.GetPublicSymbol<IMethodSymbol>()), EqualityComparer<IMethodSymbol>.Default));
...
";
            ...
            foreach (var symbol in declaredSymbols)
            {
                ...
                Assert.Equal(LocalDeclarationKind.FixedVariable, symbol.GetSymbol<LocalSymbol>().DeclarationKind);
