...
internal static class Extensions
{
    ...
    {
        return ((Microsoft.CodeAnalysis.CSharp.Symbols.PublicModel.TypeParameterSymbol)symbol).UnderlyingTypeParameterSymbol.AllEffectiveInterfacesNoUseSiteDiagnostics.SelectAsArray(t => t.GetPublicSymbol<INamedTypeSymbol>());
    }
    ...
    {
        return ((CSharpCompilation)compilation).GetSpecialTypeMember(specialMember).GetPublicSymbol<ISymbol>();
    }

    public static INamedTypeSymbol GetWellKnownType(this Compilation compilation, WellKnownType type)
    {
        return ((CSharpCompilation)compilation).GetWellKnownType(type).GetPublicSymbol<INamedTypeSymbol>();
    }

    public static ImmutableArray<IPropertySymbol> GetPublicSymbols(this ImmutableArray<PropertySymbol> symbols)
    {
        return symbols.SelectAsArray(s => s.GetPublicSymbol<IPropertySymbol>());
    }

    public static ImmutableArray<IParameterSymbol> GetPublicSymbols(this ImmutableArray<ParameterSymbol> symbols)
    {
        return symbols.SelectAsArray(s => s.GetPublicSymbol<IParameterSymbol>());
    }

    public static ImmutableArray<ITypeParameterSymbol> GetPublicSymbols(this ImmutableArray<TypeParameterSymbol> symbols)
    {
        return symbols.SelectAsArray(s => s.GetPublicSymbol<ITypeParameterSymbol>());
    }
    ...
    {
        return first.GetPublicSymbol<IAssemblySymbol>().GivesAccessTo(second.GetPublicSymbol<IAssemblySymbol>());
    }
    ...
    {
        return comp.CreateTupleTypeSymbol(underlyingType.GetPublicSymbol<INamedTypeSymbol>(), elementNames, elementLocations);
    }
    ...
    {
        return comp.CreateTupleTypeSymbol(elementTypes.SelectAsArray(e => e.GetPublicSymbol<ITypeSymbol>()), elementNames, elementLocations, elementNullableAnnotations);
    }

    public static INamedTypeSymbol Construct(this INamedTypeSymbol definition, params TypeSymbol[] typeArguments)
    {
        return definition.Construct(typeArguments.Select(s => s.GetPublicSymbol<ITypeSymbol>()).ToArray());
    }
    ...
    {
        return comp.CreateErrorNamespaceSymbol(container.GetPublicSymbol<INamespaceSymbol>(), name);
    }
